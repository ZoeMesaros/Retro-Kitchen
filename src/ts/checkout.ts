...
for (let i = 0; i < checkoutCart.length; i++) {
  ...
  selectAmount.addEventListener("change", () => {
    if (selectAmount.selectedIndex == 0) {
      const ingredientIndex = checkoutCart.findIndex(
        (item: any) => item.id === shopItems.product.id
      );
    ...
    productPrice.innerHTML = `${
      shopItems.amount * shopItems.product.product.price
    } SEK`;
    // loop and multiply with amount
...
let cartSection = document.querySelector(".checkoutCart") as HTMLElement;
